...
public class DistributorAssignedOrderAdapter extends RecyclerView.Adapter<DistributorAssignedOrderAdapter.OrderViewHolder>{

    Context context;
    boolean selectable = false;
    List<OrderModel> allOrderList;
    boolean asAssignment = false;
    ICallback iCallback;


    public DistributorAssignedOrderAdapter(Context context, List<OrderModel> allOrderList) {
        this.context = context;
        this.allOrderList = allOrderList;
    }

    public DistributorAssignedOrderAdapter(Context context, boolean asAssignment, List<OrderModel> allOrderList) {
        this.context = context;
        this.allOrderList = allOrderList;
        this.asAssignment = asAssignment;
    }

    public DistributorAssignedOrderAdapter(Context context, List<OrderModel> allOrderList, boolean selectable) {
        this.context = context;
        this.allOrderList = allOrderList;
        this.selectable = selectable;
    }

    public DistributorAssignedOrderAdapter(Context context, List<OrderModel> allNewsList, boolean selectable, ICallback iCallback) {
        this.context = context;
        this.allOrderList = allOrderList;
        this.selectable = selectable;
    ...
    @NonNull
    @Override
    public DistributorAssignedOrderAdapter.OrderViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(context).inflate(R.layout.list_order_item_main, parent, false);
        return new DistributorAssignedOrderAdapter.OrderViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull DistributorAssignedOrderAdapter.OrderViewHolder holder, int position) {

        OrderModel listOrder= allOrderList.get(position);

        /*
        String startTime = listOrder.getOrderedTime();
        ...
        try {
            ...
            holder.tvOrderedProdutConfirmedDate.setText(sdfs.format(dt));
        ...
        holder.btOrderDetails.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("assigned_order", true);
